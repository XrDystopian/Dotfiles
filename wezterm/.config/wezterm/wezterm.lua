-- WezTerm Keybindings Documentation by dragonlobster (Modified by XrDystopian)
-- ===================================================
-- Leader Key:
-- The leader key is set to ALT + s, with a timeout of 2000 milliseconds (2 seconds).
-- To execute any keybinding, press the leader key (ALT + s) first, then the corresponding key.
...
-- 5. Status Line:
--    - The status line indicates when the leader key is active, displaying a star emoji (⭐).
...
local leader_prefix = utf8.char(0x2b50)

-- Remove close confirmation prompt
...
local colors = {
	tab_bar_active_tab_fg = scheme_colors.catppuccin.mocha.pink,
...
config.leader = { key = "s", mods = "ALT", timeout_milliseconds = 2000 }
config.keys = {
	{
		mods = "LEADER",
		key = "c",
